...
`git_handoff_summary.ps1` 报告来源分支比当前 AI 分支领先多少 commit。如果开发期间来源有新提交，交接时明确警告潜在合并冲突。

### 11. Push 失败指引

`push_ai_branch.ps1` 因代理/网络问题失败时，输出 `next_action` 包含 bypass 命令（`git -c http.proxy="" push`），而不是只报错。
