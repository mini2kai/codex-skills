...
本 Skill 不固定调用任何专业 Skill。每次编排前，先根据当前会话可见的 Skill 列表读取 `name` 和 `description`，判断是否存在满足本次任务的能力。

判断顺序：

1. 先理解本次任务需要哪些能力，例如需求文档、日志、数据库、代码实施、Git 交付、文档输出。
2. 再从当前可见 Skill 中匹配 `description` 与能力需求。
3. 只使用当前确实可用、且适配本次任务的 Skill。
4. 如果没有合适 Skill，不要假装调用；明确说明缺失能力。
5. 给出严格可行的临时替代方案，并向用户索取必要信息或授权。

能力路由表只是候选，不是固定调用顺序。

## 缺失能力处理

当本地没有适配本次任务的专业 Skill 时，必须说明缺失能力、为什么现有 Skill 不适配、临时替代方案、需要用户提供什么。

缺少数据库访问能力时：

- 说明当前没有可用数据库只读查询 Skill，无法直接验证表结构或现有数据。
- 临时方案：请用户提供数据库类型、host、port、database、user、password 或一次性连接串，以及是否允许临时安装查询依赖。
- PostgreSQL 可临时使用 `psql`、`psycopg` 或 `psycopg2` 做只读查询。
- 只执行 `SELECT`、`WITH`、`SHOW`、`EXPLAIN`、表结构查看；不执行 DDL/DML/写入语句。
- 查询结束后不保存连接信息。

缺少日志读取能力时：

- 请用户提供日志片段、日志路径、服务名、时间范围、关键词、trace id 或 request id。
- 如果无法安全访问服务器，只输出可执行的检索命令和判断标准。
- 不直接执行 ssh、docker、sudo 等高风险服务器命令，除非有明确可控脚本或用户确认。

缺少 Git 交付能力时：

- 使用普通 Git 只读命令检查状态。
- 如需创建分支、commit 或 push，先输出计划并等待用户确认。
- 不执行破坏性 Git 操作，不盲目 `git add .`。

缺少代码实施专用 Skill 时：

- 使用普通 Codex 开发流程，但先明确修改范围、风险和验证计划。
- 如果涉及领域专有配置、数据库配置或外部系统规则且证据不足，必须停住要求用户补充。
...
| Git 临时分支交付 | description 包含 Git、branch、ai/*、commit、push、handoff 等能力 | 使用普通 Git 命令给计划，实施前逐步确认 |
